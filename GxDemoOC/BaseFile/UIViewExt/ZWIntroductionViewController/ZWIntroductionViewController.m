...
@property (nonatomic, strong) UIButton *beginUseBtn;
@end
...
-(void)addBeginBtn{
    ...
    [beginBtn setBackgroundColor:HEXCOLOR(0x3A90FF)];
    [beginBtn setTitle:@"开始使用" forState:0];
    [beginBtn setTitleColor:[UIColor whiteColor] forState:0];
    [beginBtn addTarget:self action:@selector(beginBtnDown) forControlEvents:UIControlEventTouchUpInside];
    beginBtn.frame = CGRectMake(AppWidth/2-50, AppHeight-70, 100, 40);
    self.beginUseBtn = beginBtn;
    [self.view addSubview:self.beginUseBtn];
    self.beginUseBtn.hidden = YES;
}
- (void)viewDidLoad {
    [super viewDidLoad];
//    [self addBeginBtn];
    ...
    if (!self.enterButton) {
        ...
        [self.enterButton setTitle:NSLocalizedString(@"开始使用", nil) forState:UIControlStateNormal];
//        self.enterButton.layer.borderWidth = 0.5;
//        self.enterButton.layer.borderColor = [UIColor whiteColor].CGColor;
        [self.enterButton setBackgroundColor:HEXCOLOR(0x3A90FF)];
        [self.enterButton setTitle:@"开始使用" forState:0];
        [self.enterButton setTitleColor:[UIColor whiteColor] forState:0];
...
- (CGRect)frameOfEnterButton
{
 ...
 //       size = CGSizeMake(self.view.frame.size.width * 0.6, 40);
  ...
  //  return CGRectMake(self.view.frame.size.width / 2 - size.width / 2, self.pageControl.frame.origin.y - size.height, size.width, size.height);
    return CGRectMake(self.view.frame.size.width / 2 - size.width / 2, AppHeight-55, size.width, size.height);
}
...
- (UIImageView*)scrollViewPage:(NSString*)imageName withTag:(NSInteger)tag
{
    
    UIImageView *imageView = [[UIImageView alloc] init];
    
    if (![imageName isEqualToString:@"1"] && ![imageName isEqualToString:@"2"] && ![imageName isEqualToString:@"3"] && ![imageName isEqualToString:@"1_4s"] && ![imageName isEqualToString:@"2_4s"] && ![imageName isEqualToString:@"3_4s"]) {
        [imageView sd_setImageWithURL:[NSURL URLWithString:imageName] placeholderImage:[UIImage imageNamed:@"LaunchImage.png"] options:SDWebImageContinueInBackground | SDWebImageDelayPlaceholder];
    } else {
...
//    self.didSelectedEnter(tap.view.tag);
}
...
- (NSArray*)scrollViewPages
{
    ...
    [self.coverImageNames enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        UIImageView *v = [self scrollViewPage:obj withTag:idx];
