...
- (void)changeStarForegroundViewWithPoint:(CGPoint)point{
    ...
    NSInteger scoreNum = score *5;
    
//    NSInteger sss = p.x / self.frame.size.height;
//    NSInteger score = sss*self.frame.size.height;
//    p.x = score;
//
//    self.starForegroundView.frame = CGRectMake(0, 0, p.x, self.frame.size.height);
//    NSInteger scoreNum = self.frame.size.width/p.x;
//
    scoreNum += 1;
    if (scoreNum<1) {
        scoreNum = 1;
...
//    NSLog(@"score:%.1f",score);
    NSLog(@"scoreNum:%ld",scoreNum);
    if(self.delegate && [self.delegate respondsToSelector:@selector(starRatingView: score:)]) {
        [self.delegate starRatingView:self score:scoreNum];
    }
}
